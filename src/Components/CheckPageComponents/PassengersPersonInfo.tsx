import { useNavigate } from "react-router-dom";
import { useAppDispatch, useAppSelector } from "../../hooks/redux";
import { resetPassengersRedux } from "../../Redux/Slice/Passengers";
import "../../styles/passengersPersonInfo.scss";

const PassengersPersonInfo = () => {
  const navigate = useNavigate();
  const dispatch = useAppDispatch();

  const { routeId } = useAppSelector((state) => state.getId);
  const info = useAppSelector((state) => state.pasenger.passengers);
  const {
    totalPrice14ClassDeparture,
    totalPrice2ClassDeparture,
    totalPrice3ClassDeparture,
  } = useAppSelector((state) => state.totalPrice);
  const { classTypeDeparture } = useAppSelector((state) => state.classType);

  const price =
    classTypeDeparture === "first" || classTypeDeparture === "fourth"
      ? totalPrice14ClassDeparture
      : classTypeDeparture === "second"
      ? totalPrice2ClassDeparture
      : classTypeDeparture === "third"
      ? totalPrice3ClassDeparture
      : 0;

  const formattedNumber = price.toLocaleString("ru-RU");

  const formatDate = (dateString: string): string => {
    if (!dateString) return "";
    const [year, month, day] = dateString.split("-");
    return `${day}.${month}.${year}`;
  };

  const handlerUpdate = () => {
    navigate(`/tickets/${routeId}/passengers`);
    dispatch(resetPassengersRedux());
  };

  return (
    <div className="passengersPersonInfo-wrapper">
      <div className="passengersInfo_title-wrapper">
        <h4>Пассажиры</h4>
      </div>

      <div className="info_pass-wrapper">
        <div className="info-wrapper">
          {info?.map((item, index) => (
            <div key={index} className="info_person-wrapper">
              <div className="avatar">
                <svg
                  width="68"
                  height="68"
                  viewBox="0 0 68 68"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M34 0C15.2492 0 0 15.2492 0 34C0 52.7508 15.2492 68 34 68C52.7508 68 68 52.7508 68 34C68 15.2492 52.7508 0 34 0ZM33.887 16.3787C38.8571 16.3787 42.9236 20.3322 42.9236 25.3023C42.9236 30.2724 38.9701 34.3389 34 34.3389C29.0299 34.3389 24.9635 30.2724 24.9635 25.3023C25.0764 20.4452 29.0299 16.3787 33.887 16.3787ZM51.9601 52.186C39.8738 52.186 28.1262 52.186 16.2658 52.186C15.701 46.5382 15.701 44.392 21.3488 41.6811C29.7076 37.7276 38.2924 37.7276 46.7641 41.6811C47.8937 42.1329 48.9103 42.9236 49.8139 43.6013C51.2824 44.8439 52.0731 46.4252 51.9601 48.3455C51.9601 49.701 51.9601 50.8306 51.9601 52.186Z"
                    fill="#FFA800"
                  />
                </svg>
                <span>{item.personInfo.is_adult ? "Взрослый" : "Детский"}</span>
              </div>
              <div className="info_person-description">
                <h4 className="title_fio">
                  {`${item.personInfo.last_name} 
              ${item.personInfo.first_name} 
              ${item.personInfo.patronymic}`}
                </h4>
                <span>
                  Пол {item.personInfo.gender ? "мужской" : "женский"}
                </span>
                <span>
                  Дата рождении {formatDate(item.personInfo.birthday)}
                </span>
                <span>
                  {`${
                    item.personInfo.document_type === "passport"
                      ? "Паспорт"
                      : "Сведетельство о рождении"
                  }
             
             
             ${item.personInfo.document_data}`}
                </span>
              </div>
            </div>
          ))}
        </div>
        <div className="info_total_price-wrapper">
          <div>
            <span>Всего</span>
            <span>
              {formattedNumber}
              <span> &#8381;</span>
            </span>
          </div>

          <button className="check_update-btn" onClick={handlerUpdate}>
            Изменить
          </button>
        </div>
      </div>
    </div>
  );
};

export default PassengersPersonInfo;
