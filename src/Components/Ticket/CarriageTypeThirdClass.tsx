...
import { CarriageThirdClass } from "./CarriageThirdClass";
...
export const CarriageTypeThirdClass: React.FC<CarriageTypeClassProps> = ({
  direction,
}) => {
  ...
  const taken = 54 - result.availableCount; // количество уже занятых мест
  ...
  useEffect(() => {
    ...
    dispatch(
      setPriceClass({
        direction: "departure",
        index: 3,
        value: selectedCoach.coach.side_price,
      })
    );
  }, [direction, selectedCoach, seatsDeparture]);

  const formattedIndex = (selectedCoachIndex + 10).toString().padStart(2, "0");
  return (
    <div className="carriage">
      ...
      <CarriageThirdClass
        ...
        taken={taken}
        carriageNum={selectedCoachIndex + 10}
        direction={direction}
      />
    </div>
  );
